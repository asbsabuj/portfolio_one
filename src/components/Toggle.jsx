...
const Toggle = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const darkThemeParser = darkTheme !== undefined && JSON.parse(darkTheme)
    const systemTheme =
    ...
    if (darkTheme === undefined) {
      ...
      darkThemeParser ? addDarkTheme() : removeDarkTheme()
    }
  }, [])
  ...
  return (
    <main ref={mainRef}>
      <div className="bg-zinc-50 dark:bg-zinc-800">
        <div
          className="max-w-[1200px] xl:w-full mx-auto flex justify-center xl:px-[90px] sm:pl-[80px]
        sm:pr-5 overflow-hidden "
        >
          <button
            onClick={() => {
              if (!darkTheme) {
